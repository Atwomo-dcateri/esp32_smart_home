...
void bsp_display_show_status(const char *status) { // 创建/更新状态栏文本
    ...
    if (status_label == NULL) { // 第一次调用：尚未创建 label
        ...
        lv_obj_set_style_text_font(status_label, &lv_font_montserrat_10, 0); // 设置字体（传入字体对象地址）
...
//     if (lvgl_port_lock(-1))
//     {
//         lv_obj_t *scr = lv_screen_active(); // 为scr找一个父对象
//         lv_obj_set_style_bg_color(scr, lv_color_black(), 0);  // 设置屏幕背景颜色为黑色

//         // 标签位置位于屏幕顶部
//         ui_header_bar = lv_obj_create(scr); // 在屏幕上挂一个标签
//         lv_obj_set_size(ui_header_bar, 128, 16);
//         lv_obj_align(ui_header_bar, LV_ALIGN_TOP_MID, 0, 0);// 设置标签位置
...
//         // MQTT状态图标（初始状态为断开）
//         ui_mqtt_icon = lv_obj_create(scr);
//         lv_label_set_text(ui_mqtt_icon, "");
//         lv_obj_set_style_text_color(ui_mqtt_icon, lv_color_white(), 0);
...
//         lv_obj_set_style_opa(ui_main_cont, 0, 0); // 只留一条分割线
